...
public class ProfilesRestController {
    ...
    public ResponseEntity<List<Profile>> getProfiles(@PathVariable(value = "profileSystem") String profileSystem,
        ...
        //Long count = service.getProfilesCount();
        if (profileSystem.equals("all")) {
            return new ResponseEntity<>(service.getAllProfiles(curPageSize, page), HttpStatus.OK);
        } else {
            return new ResponseEntity<>(service.getAllProfilesBySystem(profileSystem, curPageSize, page), HttpStatus.OK);
        }
    }
}
